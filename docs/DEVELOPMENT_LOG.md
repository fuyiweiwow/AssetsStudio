...
| 2026-08-14 | implementation/preview | F008/garments | Added a pure-2D movement test: export eight Actor walk-pose bone frames, then draw Recipe-driven robe/sleeve deformation with Pillow into four directions and review GIFs; no 3D garment is rendered | Validate that motion can be driven by existing Actor animation data without cloth simulation or a runtime 3D garment; retain the simplified body as a diagnostic proxy until approved 2D Actor layers are connected |

...
| 2026-08-14 | validation/diagnostic | F008/garments | Extended v2 simulation reached frame 99 but failed static equilibrium, cloth-body intersection, and cloth-self intersection with 1,946 self-collisions and 72 body collisions | Keep the OBJ diagnostic-only; test reduced lower flare plus a small connection clearance before considering Actor transfer |
| 2026-08-14 | validation/decision | F008/garments | Two-panel v3 lowered self-collisions to 1,545 and body collisions to 36 but still missed the physical gates; the first GarmentCode-only tuning loop is closed | Move the next prototype step to a recipe-driven Blender silhouette/render layer for the 2D target, while keeping non-equilibrated simulation output diagnostic-only |
| 2026-08-14 | implementation/preview | F008/garments | Added and verified `build_parametric_robe_2d_prototype.py` using the v3 Recipe; it emits a shoulder-anchored robe body, two visible long sleeves, four rest-pose review PNGs, a Blender candidate, and an explicit diagnostic manifest | Establish the smallest usable 2D workflow step before adding the hood or decorative layers; keep it `2d_silhouette_only` and outside formal garment promotion |
| 2026-08-14 | implementation/preview | F008/garments | Replaced the rejected pure-2D movement path with a 3D rigged robe prototype; deterministic Pelvis/Spine and Upperarm/Forearm/Hand weights preserve the Actor Walk action, and Eevee renders four directions x eight frames with Recipe lighting | Keep the final chain as 3D garment deformation and lighting followed by 2D frame output; retain `review_required` and `not_simulated` until torso exposure, sleeve attachment, hem continuity, and hood gates pass |

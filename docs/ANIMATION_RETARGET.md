...
4. 自动检查映射完整度、帧范围、手臂和大腿动作幅度、左右手次序、双手前后反相，以及“双手同时绕到背后”的帧占比。
...
Mixamo 源骨架的 rest pose 是 T-pose，而当前 Actor 已经是 Relaxed-A。身体和腿使用 rest-basis 映射；手臂只提取源动画的前后摆动相位，再以 Actor 自己的肩、肘、腕中立姿势重建。禁止把 T-pose 到 Walk 的手臂偏移再次叠加到 Actor，否则双手可能同时进入背后甚至左右交叉。

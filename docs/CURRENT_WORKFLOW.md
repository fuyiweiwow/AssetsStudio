...
当前实验停在“无部件 Actor Core 图像”之前的二维造型 Gate。2026-09-07 已证明同图局部去眼眉可以保形，但首轮 A 的头、躯干与四肢风格被人工否决；因此暂停 B、四向、3D、AccuRIG、动作和配件推进。当前先保持原始概念图的自然垂臂姿态，用原图像素直接控制带眼眉 A 的头部饱满度、头身比、躯干体量和四肢长度；自然姿态通过后才隔离验证 T Pose。该临时顺序与验收见 `docs/ACTOR_CORE_2D_FIRST_PLAN.md`；下面已经完成的 3D、绑定和配件条目都是技术证据，不代表当前下一步。
...
1. 用户已选择自然垂臂候选 83 方向；当前人工并排评审 83 与像素级微调重绘候选 93。93 将头高降低约 `4.1%`、手端抬到胯点上方并轻微收窄下肢，仍保留耳朵，不是 A 定稿。
2. 若 93 的比例方向通过，只隔离处理耳朵和残余语义边界，完成一枚自然姿态 A；A 通过后再做局部去眼眉 B。失败图不进入四向、3D、训练 Target 或资产库。
3. A/B 通过后，才从同一形态权威隔离转换 T Pose；姿态导引只提供关节位置，不再定义身体轮廓。
...
6. 新基准素体通过形体后才恢复一对一 AccuRIG、动作适配和 Slot 配件流程；真实 RTX 3060 12GB 本地推理 Gate 仍必须单独完成。

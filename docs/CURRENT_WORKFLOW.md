...
1. 人工审阅 Studio 中唯一可见的 3D 候选 `actor_core_v6_seed20260867_hy3d_v1`；只可确认入库或销毁，不得跳过五项人工 Gate。
2. 若确认，先为该网格生成专属的低风险绑定网格与 AccuRIG 输入；旧 Actor 的骨骼不得复制到新网格。
3. 用户在 AccuRIG 完成该模型的一对一标定和 FBX 导出后，由 Studio intake 处理并生成四向绑定预览。
...
- 当前选择 seed `20260867` 作为唯一可见 3D 评审候选：93,438 顶点、186,872 面、单连通、watertight、法线一致、Euler=2；四向平均权威轮廓 IoU `0.9380`，最大相对宽高比漂移 `3.96%`。轮廓阈值尚未用足够正反样本校准，因此该数值只做排序，不能自动批准。
- Hunyuan3D 20-step 生成峰值显存为 `5,828,215,296` bytes（约 5.43 GiB），表明 3060 具备形状阶段候选可行性，但真实 3060 运行仍未完成。
- 旧 Actor `0ef398ca...` 已从 Studio 资产列表隐藏，只保留 AccuRIG、走路动作和生命周期技术基线文件；新候选未接受前，Studio 资产库对当前权威 Actor 显示为空。

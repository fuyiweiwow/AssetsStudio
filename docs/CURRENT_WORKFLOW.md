...
当前实验已从“无部件 Actor Core 图像”的正面 T Pose Gate 进入第一个独立多视图 Gate。用户指出右侧 seed 1005 的腹部过凸、眼部未清除，同时确认浅鼻部轮廓可以保留、手部本轮暂缓。修订版在同一冻结源上确定性去除眼眉和内耳，并把腹部前缘最多收回 `17 px`；没有重新生成头、四肢或改变身高。右侧 B 的自动对齐 Gate 全部通过，但仍为 `human_review_required`。审核前暂停背面、3D、AccuRIG、动作和配件推进。该临时顺序与验收见 `docs/ACTOR_CORE_2D_FIRST_PLAN.md`；下面已经完成的 3D、绑定和配件条目都是技术证据，不代表当前下一步。
...
2. 审核 `workspace/local_generation/actor_core_multiview_gate_20260907/right_b_seed1005_refined_review.png`：确认眼眉和内耳已无语义残留、浅鼻部轮廓可接受、腹部不再过凸，并复查后脑及躯干侧向厚度。手/肩侧投影按用户意见暂缓，但必须在 3D 拓扑前回查。
3. 当前修订版的头部侧深/正面宽为 `1.006`、躯干侧深/正面宽为 `0.812`，自动对齐通过不能替代人工造型判断。
4. 右侧 B 人工通过后再隔离生成 back；姿态与视角导引只能约束关节和投影，不允许各方向独立生成不同角色或重定义身体比例。

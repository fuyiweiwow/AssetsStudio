...
## 最新停点：2026-09-09 脚底修复后人工标定

脚底内侧薄片已做局部修复，旧 50k/FBX 不再作为本轮标定输入。请使用修脚版 `Actor_Offline_v2_FootRepair_AccuRIG.fbx`；通过结构和交换文件检查，仅许可实验性标定，之后验证动作。文件、复现步骤及剩余限制见 [脚底修复交接](ACTOR_FOOT_REPAIR_20260909.md)。以下先前阶段记录不覆盖此停点。

**换机入口：** [2026-09-08 阶段成果与家庭环境重放](ACTOR_HOME_CHECKPOINT_20260908.md)。冻结文件有相对路径/SHA256 清单，支持不加载 AI 模型的静态一键重放；RTX3060 三维生成资格单独验证。

2026-09-08 未绑定最新：50000 面素体副本已通过导出后结构门，四向轮廓平均 IoU 0.999860。腰带现采用独立的接触间隙/前侧外凸合同，`waist_contact_v5` 通过有限静态检查及双向包含抽样，待人工外观审核，尚未入库、绑定或接入 Studio。原 AABB 失败记录保留；新合同不是扩大旧门限。详情见 [腰带接触适配](WAIST_CONTACT_FIT_20260908.md)，此前测量/减面见 [未绑定交接](ACTOR_OFFLINE_STATIC_HANDOFF_20260908.md)。

2026-09-08 最新检查点：已重建离线 RGBA 预处理与结构检查入口。原来的外轮廓填充把腿间背景误当实体；新 profile 保留腿缝及头部浅色高光，相同 Hunyuan seed/40 步重跑后，原始网格单连通、封闭、Euler=2、7 个脚部截面分离，结构门全部通过。没有手工网格修补。当前仍为待人工形态审核候选，不绑定或入库；脚缘/手部细节、3060 实机和多 Actor 通用性尚未验收。复现与范围见 [离线流程](OFFLINE_ACTOR_CORE_REPAIR.md)，历史对照见 [Actor93 形体对照](ACTOR93_SHAPE_PROBE_20260908.md)。下方较早停点仅为历史记录，以本段为当前状态。
...
当前实验已进入“无部件 Actor Core 图像”的背面 Gate。用户已人工确认腹部收窄、去眼眉/内耳且保留浅鼻部轮廓的右侧 B 可以继续。背面 seed 1009 因头宽比正面大约 `15%` 被拒绝；seed 1008 的比例和落点稳定，经确定性平滑去除背中线与臀沟后，全部自动对齐 Gate 通过，当前为 `human_review_required`。背面人工通过前暂停左侧冻结、3D、AccuRIG、动作和配件推进。该临时顺序与验收见 `docs/ACTOR_CORE_2D_FIRST_PLAN.md`；下面已经完成的 3D、绑定和配件条目都是技术证据，不代表当前下一步。
...
2. 审核 `workspace/local_generation/actor_core_multiview_gate_20260907/back_b_seed1008_review_v1.png`：检查背面头部是否圆而不过宽、背部和骨盆是否真正无中线/臀沟、圆肩和开放腋下是否自然；手形按用户意见暂缓，但必须在 3D 拓扑前回查。
3. 当前背面相对正面的头宽比 `1.006`、头高比 `1.005`、臂展比 `1.004`、躯干宽比 `1.007`，左右手端高度差 `1 px`；自动对齐通过不能替代人工造型判断。
4. 背面 B 人工通过后，由已批准右侧 B 做确定性水平镜像得到左侧 B；对称无部件素体不再消耗一次独立生成，避免身份和比例漂移。
5. 四向通过后冻结源图哈希与人工结论，再建立唯一 3D 基准素体；现有 TripoSG/UniRig/Hunyuan3D 结果只复用技术验证方法。
6. 新基准素体通过形体且手/肩拓扑回查后，才恢复一对一 AccuRIG、动作适配和 Slot 配件流程；真实 RTX 3060 12GB 本地推理 Gate 仍必须单独完成。
...
- 用户已允许 5070 Ti 继续制作教师资产，因此固定二维通过图已进入 Hunyuan3D-2MV。seed `20260865` 因头身为两个连通体被拒绝；seed `20260866` 与 `20260867` 均为单一封闭网格。
- 当前选择 seed `20260867` 作为实验可用 3D Actor：93,438 顶点、186,872 面、单连通、watertight、法线一致、Euler=2；四向平均权威轮廓 IoU `0.9380`，最大相对宽高比漂移 `3.96%`。头顶存在轻微低频起伏，因此只批准用于绑定与模块配件工作流实验，尚不是最终生产 canonical。
- Hunyuan3D 20-step 生成峰值显存为 `5,828,215,296` bytes（约 5.43 GiB），表明 3060 具备形状阶段候选可行性，但真实 3060 运行仍未完成。
- 新 Actor 的非破坏性绑定副本为 61,002 顶点 / 122,000 面，仍为单连通、封闭、流形；与原网格四向最低轮廓 IoU `0.999967`。专属 AccuRIG FBX 已通过往返 Gate，等待人工标定。
- 旧 Actor `0ef398ca...` 已从 Studio 资产列表隐藏，只保留 AccuRIG、走路动作和生命周期技术基线文件；其骨骼不能用于新 Actor。

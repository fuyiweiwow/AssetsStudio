...
当前实验停在“无部件 Actor Core 图像”之前的二维造型 Gate。用户已确认自然垂臂候选 93 可作为比例/形态基准；同一像素基准已分层得到 A（无耳、保留眼眉）和 B（无耳、无五官），自动保持性 Gate 通过，正在等待人工 A/B 审核。审核前暂停 T Pose、四向、3D、AccuRIG、动作和配件推进。该临时顺序与验收见 `docs/ACTOR_CORE_2D_FIRST_PLAN.md`；下面已经完成的 3D、绑定和配件条目都是技术证据，不代表当前下一步。

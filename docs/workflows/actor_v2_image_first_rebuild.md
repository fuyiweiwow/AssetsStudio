...
- Current state: `rig_actions_head_feature_calibration_v1_fit29_technical_pass_visual_review_required_remaining_slots_next`
- Actor V1 remains the accepted game Actor.
- Actor V2 must restart from an approved image-first source; the currently accepted source originated from `image_gen`. A local backend may replace or extend it only after the same visual and multiview gates pass. No manual proportion morph may enter the source-of-truth chain.
- The approved visual source has passed local Hunyuan shape reconstruction, Blender static multiview QC, AccuRIG binding and Walk/Run retarget stress tests. The current `head_feature_calibration_v1` replay candidate (`fit29`) passes EyeAssembly/blink, calibrated detachable Miku `EarPair`, source-locked default `head_hair` and action gates, but still requires final visual approval. Default `torso_outer`, `waist_accessory` and `legs_outer` also pass; three visual slots (`feet_outer`, `wrist_accessory`, `back_accessory`) and final runtime assembly remain pending.
...
The validated local candidate for deriving the missing views is `approved front anchor -> FLUX.2 Klein ReferenceLatent -> view classification -> panel registration/reorder -> multiview QA`. It is documented in `docs/workflows/local_reference_turnaround_validation_2026-08-23.md`. Prompted panel order is not authoritative; classify the actual views before naming or feeding Hunyuan.
...
Actor V2 now uses the retained two-surface `EyeAssemblyV1` structure with independent `open / half / closed` texture states. Placement must be resolved from the current head instead of copied from Actor V1 or forced to symmetric world coordinates. `head_feature_calibration_v1.json` measures the weighted, evaluated head bounds and resolves character-left/right X centers `+0.215579 / -0.147054 m`, eye center Z `1.386420 m`, surface width `0.229963 m`, height `0.279559 m` and clearance `0.006 m` for this Actor. The asymmetric values are intentional: the measured head center is `X=+0.034263 m`. Actor L maps to viewer-named `eye_right` and Actor R to `eye_left`, so both outer lashes point outward. The builder now removes old objects and orphaned state materials before replay, preserving canonical material names. The assembly is bone-parented to `CC_Base_Head`, follows Walk, renders no eye geometry from the back, and passes four directions x eight deterministic blink frames. Final fit29 eye-surface median distances are `1.7668 mm` (L) and `1.7601 mm` (R).
...
When automatic placement is visually close but not art-directable enough, the Studio face workbench provides an optional manual feedback layer. `TransformControls` uses a proxy pivot at each visible mesh bounding-box centre rather than the GLB node origin, because the web eye meshes contain baked world-space geometry. Exact `1 mm / 1%` nudges and mirrored two-eye linkage are the preferred final-placement controls. Studio exports only relative `assetsstudio_head_feature_feedback_v1` adjustments. `apply_studio_head_feature_feedback.py` converts Three.js X-right/Y-up/Z-out deltas into Blender coordinates, preserves `CC_Base_Head` parenting, and the canonical PowerShell replay reruns every existing gate when `-StudioFeedback` is supplied. Manual review is therefore an auditable override, not a replacement for fitting or validation.
...
The procedural UV-sphere ear draft and the later Hunyuan geometric-ear branch are revoked for the current Actor. The retained default bundle is the previous Miku source pair from `milestones/body/chibi_actor_eye_assembly_v2.blend`, rebuilt as two independent `MikuEar_*_SourceV1` Slot objects. Each ear root is resolved independently from `head_feature_calibration_v1.json` and projected to the current head surface with `0.004 m` clearance; root medians remain about `0.004 m` after replay. The root stays fixed while outward scale `2.0` makes the detachable ear readable through the paired side hair. Both objects follow `CC_Base_Head`; head-relative translation drift across frames `1 / 31 / 71` is `0.00003105 m`, below the `0.0001 m` gate. Deleting a runtime ellipsoidal hole in the hair is prohibited because fit21/fit22 exposed jagged scalp boundaries.

## Default head-hair result

User review revoked the procedural `head_hair/default_adventurer_v2_layered` replacement. The default hair must originate from the saved two-stage image-first workflow: place hair over each exact Actor calibration view, remove the Actor while preserving the wearable shell and ear openings, then run standard Hunyuan3D-2MV.

The Stage 10 history restored the exact former successful source set and `adventurer_head_hair_actorfit_2mv_v2.glb`. That source used four independent images, seed `20260820`, 30 steps, guidance `5.0` and octree `256`. Raw Blender validation reports `327,377` vertices / `654,738` faces. The retained review source is `workspace/actor_v2/slots/head_hair/paired_v2_clean/head_hair_paired_v2_review128k.blend`, with `63,991` vertices / `127,998` faces.

The raw recovered mesh contains recognizable paired locks but an incomplete smooth base shell; fit12 therefore exposed large scalp regions and is rejected. The corrected adapter reads the same head calibration used by eyes and ears, then combines the source-locked visible locks with a bounded undercap copied from the current evaluated Actor scalp. The undercap is an interface layer only: no replacement locks are authored. Fit29's rectangular front cutoff passed the old coverage metric but exposed a straight brown strip across the forehead and was visually rejected as a headband. The current adapter uses a high-centre/low-temple `curved_source_bangs_occlusion_v1` front boundary hidden behind the source bangs, and the coverage validator measures only the same curved intended-hair region. Parameters remain width ratio `1.10`, Q-height ratio `1.15`, top clearance `0.06 m`, radial clearance `0.09 m`, cap bottom offset `0.44 m` and orthographic shell offset `0.055 m`. The repaired candidate at `workspace/actor_v2/head_feature_hairline_fix/actor_head_complete_calibrated.blend` retains the `0.981132` minimum four-view coverage and passes static close-ups, deterministic blink and Walk `1-71` x four directions/eight frames.

The canonical replay entry point is `tools/build_actor_v2_head_features.ps1`. It accepts a rigged Actor Blend, recalibrates the current evaluated head, rebuilds eye materials idempotently, fits Miku ears, fits paired hair, optionally applies Studio feedback, and executes distance, coverage, blink and action validation. A new Actor must use this replay; copying any prior Actor's world coordinates is forbidden.

A fresh two-stage source set was also generated and run through standard 2MV at 30 steps/guidance `5.0`/octree `256`, but its raw lower shell still melted because independently generated lock topology did not correspond tightly enough. It is rejected before fitting. Full evidence and reusable gates are in `docs/quality/ACTOR_V2_DEFAULT_HAIR_REBUILD_2026-08-22.md`.
...
2. Generate and review one assembled default-adventurer turnaround in front/right/back/left order. The current identity master is `references/actor_v2/actor_v2_default_adventurer_turnaround_v1_candidate.png`. For a local rerun, FLUX.2 Klein ReferenceLatent may derive the missing views from an approved front anchor, but the actual panel orientations must be classified and registered before use.
...
6. Load the passing FBX in AccuRIG, manually confirm pelvis/spine/neck/head, shoulders/elbows/wrists, hips/knees/ankles/toes, select zero fingers, bind skin and export the rigged FBX. After re-import, run `tools/build_actor_v2_head_features.ps1` so `EarRoot_L/R`, eye anchors and hair coordinates are derived from the current evaluated head rather than copied. **Completed for the current Actor; Walk, Run, four-weight optimization and fit29 EyeAssembly/blink gates pass.**
7. Isolate the default `EarPair` and seven visual slots (hair plus six wearables) from the approved master while preserving front/right/back/left correspondence. **The calibrated Miku `EarPair`, source-locked `head_hair`, `torso_outer`, `waist_accessory` and `legs_outer` are technical-pass candidates; fit29 head assets still require final visual approval.**
8. Process each slot through RGB/RGBA, a validated geometry source, the smallest slot-specific adapter and static/action QA. For the default hair and ears, visible geometry must remain image/Hunyuan-derived; procedural geometry may only serve a documented interface adapter. Raw canonical geometry must pass before fitting. **Next: `feet_outer`, because it owns the lower-leg/ground interface.**
